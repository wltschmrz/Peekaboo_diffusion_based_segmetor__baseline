...
# This code is based on the Stable-Dreamfusion codebase's 'sd.py' by Jiaxiang Tang
# https://github.com/ashawkey/stable-dreamfusion
# which is licensed under the Apache License Version 2.0. It has been
# heavily modified to suit Peekaboo's needs, but the basic concepts remain the same.
...
class StableDiffusion(nn.Module):
    def __init__(self, token, device='cuda', checkpoint_path="CompVis/stable-diffusion-v1-4", variant="fp16"):
        ...
        assert isinstance(self.tokenizer, CLIPTokenizer), type(self.tokenizer)
        assert isinstance(self.text_encoder, CLIPTextModel), type(self.text_encoder)
    ...
    def get_text_embeddings(self, prompts: Union[str, List[str]]) -> torch.Tensor:
        ...
        uncond_input = self.tokenizer([self.uncond_text] * len(prompts), padding='max_length',
    ...
    def train_step(self, text_embeddings: torch.Tensor, pred_rgb: torch.Tensor, guidance_scale: float = 100,
                   t: Optional[int] = None):
    ...
    def produce_latents(self, text_embeddings: torch.Tensor, height: int = 512, width: int = 512,
                        num_inference_steps=50, guidance_scale=7.5, latents=None) -> torch.Tensor:
        ...
        with torch.autocast('cuda'):
            for i, t in enumerate(self.scheduler.timesteps):
                assert int(
                    t) == t and 0 <= t <= 999, 'Suprisingly to me...the timesteps were encoded as integers lol (np.int64)'
                ...
                latent_model_input = torch.cat(
                    [latents] * 2)  # The first half is the blank prompts (repeated); the second half is
                ...
                latents = self.scheduler.step(noise_pred, t, latents)[
                    'prev_sample']  # It's a dict with nothing but 'prev_sample' in it
